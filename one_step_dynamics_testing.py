...
def main():
    ...
    print("Evaluating short-horizon prediction errors on test batch (Spiking Regions & Chebyshev Norm)...")
    ...
    subspace_cheb_errors = []
    potential_proj_cheb_errors = []
    potential_act_cheb_errors = []
    ...
    for n in range(1, 6):
        ...
        mask = (v_targets_actual >= args.spike_threshold)
        
        abs_sub_diff = jnp.abs(z_preds_all - z_targets_all) * mask[:, jnp.newaxis, :]
        abs_proj_diff = jnp.abs(v_preds_all - v_targets_reconstructed) * mask
        abs_act_diff = jnp.abs(v_preds_all - v_targets_actual) * mask
        
        sub_cheb = float(jnp.max(abs_sub_diff))
        pot_proj_cheb = float(jnp.max(abs_proj_diff))
        pot_act_cheb = float(jnp.max(abs_act_diff))
        
        subspace_cheb_errors.append(sub_cheb)
        potential_proj_cheb_errors.append(pot_proj_cheb)
        potential_act_cheb_errors.append(pot_act_cheb)
        
    print("\nShort-Horizon Chebyshev Norm Errors on Spiking Regions (v >= 0.0):")
    for n in range(5):
        print(f"Horizon Step {n+1:d} | Subspace Chebyshev: {subspace_cheb_errors[n]:.6e} | Projected Potential Chebyshev: {potential_proj_cheb_errors[n]:.6e} | Actual Potential Chebyshev: {potential_act_cheb_errors[n]:.6e}")
        
    print("\nMPC Suitability Analysis (Chebyshev Norm):")
    if potential_act_cheb_errors[0] < 1e-4:
        print("  - One-step potential worst-case error is exceptionally small (< 10^-4).")
        print("  - The Bilinear DMDc model captures the one-step spiking dynamics perfectly.")
    else:
        print(f"  - One-step potential worst-case error is: {potential_act_cheb_errors[0]:.6e}")
        
    if potential_act_cheb_errors[4] < 1e-2:
        print("  - 5-step recursive worst-case prediction error is extremely small (< 10^-2).")
        print("  - The learned operators are highly suited for real-time Model Predictive Control (MPC).")
    else:
        print(f"  - 5-step recursive worst-case potential error is: {potential_act_cheb_errors[4]:.6e}")
    ...
    if not args.no_plot:
        ...
        steps = np.arange(1, 6)
        ax1.plot(steps, subspace_cheb_errors, 'o-', color='#1f77b4', linewidth=2.0, markersize=8)
        ax1.set_yscale('log')
        ax1.set_title("Subspace Chebyshev Error (Spiking)", fontsize=13, fontweight='bold')
        ax1.set_xlabel("Horizon Steps Ahead (n)", fontsize=11)
        ax1.set_ylabel("Subspace Chebyshev Error (log scale)", fontsize=11)
        ax1.set_xticks(steps)
        ax1.grid(True, which="both", linestyle='--', alpha=0.6)
        
        ax2.plot(steps, potential_act_cheb_errors, 's--', color='#d62728', linewidth=2.0, markersize=8)
        ax2.set_yscale('log')
        ax2.set_title("Membrane Potential v Chebyshev Error (vs Actual)", fontsize=13, fontweight='bold')
        ax2.set_xlabel("Horizon Steps Ahead (n)", fontsize=11)
        ax2.set_ylabel("Potential Chebyshev Error (log scale)", fontsize=11)
        ax2.set_xticks(steps)
        ax2.grid(True, which="both", linestyle='--', alpha=0.6)
        
        plt.suptitle(f"Bilinear DMDc Short-Horizon Chebyshev Error (Spiking Regions)\n(Evaluated on {args.test_batch} randomized test trajectories)", 
                     fontsize=15, fontweight='bold', y=0.98)
        plt.tight_layout()
        save_path = os.path.join(plots_dir, 'fhn_short_horizon_errors.png')
        plt.savefig(save_path, dpi=300)
        print(f"\nSaved Chebyshev error scaling visualization plot to {save_path}")
        plt.close()
